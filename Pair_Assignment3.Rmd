...
SparkassenBoard_UniqueMembers <- unique(SparkassenBoard[,c("NameCandidate1", "occupation", "Incumbent", "NameMunicipality")])
## :mayors at board
...
```{r Comparison: Board Members in Entire board and Top Positions, include=TRUE, echo= FALSE}
...
```{r graphs_election_dataset, include=TRUE, echo=FALSE}
...
#   if(MayorElection$MayorParty[i] == "NA") {
#     next
#   }
#   if(MayorElection$MayorParty[i] == "CSU") {
#     next
#   }
#   if(MayorElection$MayorParty[i] == "CSU/BU") {
#     next
#   }
#   if(MayorElection$MayorParty[i] == "PWG") {
#     next
#   }
#   if(grepl("Wähler", MayorElection$MayorParty[i])) {
#     MayorElection$MayorParty[i] <- "Free Voters"
#   }
# }
#
#
# barplot(table(MayorElection$"Wahlvorschlag 1"))
```



Out of all the mayors elected over the entire time period, `r round((table(MayorElection$Geschlecht1)[2] / nrow(MayorElection))*100, 3)`% of the mayors who were elected were female.
